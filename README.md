...
- remembered physical device inventory
...
6. Flush data with `fsync` and `F_FULLFSYNC` where available.
7. Attempt to remount writable partitions.
8. Write a new `/.uuid` JSON file if a writable mounted partition is available.
9. Persist image usage, device inventory, and flash history.
...
- There is no full read-back verification pass yet.
